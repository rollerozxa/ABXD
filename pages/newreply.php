...
if ($loguser['powerlevel'] < 0)
	Kill("You're banned. You can't post.");
...
$crumbs->add(new PipeMenuTextEntry("New reply"));
...
if (isset($_POST['actionpreview'])) {
	...
	if (!$_POST['text']) {
		...
		Alert("You can't double post until it's been at least one day.", "Sorry");
		$rejected = true;
	}
	else {
	...
	if (!$rejected) {
		...
		logAction('newreply', ['forum' => $fid, 'thread' => $tid, 'post' => $pid]);
...
if (CanMod($loguserid, $fid)) {
	...
	if (!$thread['closed'])
		...
		$mod .= "<label><input type=\"checkbox\" ".getCheck("unlock")."  name=\"unlock\">&nbsp;Open thread</label>\n";

	if (!$thread['sticky'])
		$mod .= "<label><input type=\"checkbox\" ".getCheck("stick")."  name=\"stick\">&nbsp;Sticky</label>\n";
	else
		$mod .= "<label><input type=\"checkbox\" ".getCheck("unstick")."  name=\"unstick\">&nbsp;Unstick</label>\n";
