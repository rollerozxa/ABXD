...
if (!$thread['sticky'] && Settings::get("oldThreadThreshold") > 0 && $thread['lastpostdate'] < time() - (2592000 * Settings::get("oldThreadThreshold")))
	$replyWarning = " onclick=\"if (!confirm('Are you sure you want to reply to this old thread? This will move it to the top of the list. Please only do this if you have something new and relevant to share about this thread's topic that is not better placed in a new thread.')) return false;\"";
if ($thread['closed'])
	$replyWarning = " onclick=\"if (!confirm('This thread is actually closed. Are you sure you want to abuse your staff position to post in a closed thread?')) return false;\"";

$links = new PipeMenu();
if ($loguserid) {
	if ($loguser['powerlevel'] < 0)
		$links -> add(new PipeMenuTextEntry("You're banned."));
	else if (IsAllowed("makeReply", $tid) && (!$thread['closed'] || $loguser['powerlevel'] > 2))
		$links -> add(new PipeMenuLinkEntry("Post reply", "newreply", $tid, "", "comment"));
	...
	if (CanMod($loguserid,$forum['id']) && IsAllowed("editThread", $tid)) {
		$links -> add(new PipeMenuLinkEntry("Edit", "editthread", $tid, "", "pencil"));
		if ($thread['closed'])
			$links -> add(new PipeMenuLinkEntry("Open", "editthread", $tid, "action=open&key=".$loguser['token'], "unlock"));
		else
			$links -> add(new PipeMenuLinkEntry("Close", "editthread", $tid, "action=close&key=".$loguser['token'], "lock"));
		if ($thread['sticky'])
		...
		if ($forum['id'] != Settings::get('hiddenTrashForum'))
			$links -> add(new PipeMenuLinkEntry("Delete", "editthread", $tid, "action=delete&key=".$loguser['token'], "remove"));
...
if ($thread['poll']) {
	...
	$voters = $poll["users"];
	$bottom = format($voters == 1 ? "{0} user has voted so far." : "{0} users have voted so far.", $voters);
	if ($poll["doublevote"])
		$bottom .= " ".format("Total votes: {0}.", $poll["votes"])." Multi-voting is enabled.";
...
if ($pagelinks) write("<div class=\"smallFonts pages\">Pages: {0}</div>", $pagelinks);
