...
if ($loguserid) {
	...
	if ($isIgnored)
		$links->add(new PipeMenuLinkEntry("Unignore forum", "forum", $fid, "unignore", "eye-open"));
	else
		$links->add(new PipeMenuLinkEntry("Ignore forum", "forum", $fid, "ignore", "eye-close"));

	if ($forum['minpowerthread'] <= $loguser['powerlevel'])
		$links->add(new PipeMenuLinkEntry("Post thread", "newthread", $fid, "", "comment"));
}
...
if (NumRows($rThreads))
	...
	elseif ($loguserid)
		Alert(format("Would you like to {0}?", actionLinkTag("post something", "newthread", $fid)), "Empty forum");
	else
		Alert(format("{0} so you can post something.", actionLinkTag("Log in", "login")), "Empty forum");
