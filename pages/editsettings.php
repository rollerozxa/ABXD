...
if ($loguser['powerlevel'] < 3)
	Kill("You must be an administrator to edit the board settings.");
...
if ($plugin == "main")
	$crumbs->add(new PipeMenuLinkEntry("Edit settings", "editsettings"));
else {
...
if (isset($_POST["_plugin"])) {
	...
	if ($valid) {
		...
		if ($plugin == "main")
			logAction('editsettings', []);
		else
			logAction('editplugsettings', ['text' => $plugin]);
...
print "			<tr class=\"cell2\">
				...
				<td>
					<input type=\"submit\" name=\"_exit\" value=\"Save and Exit\">
