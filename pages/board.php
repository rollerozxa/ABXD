...
$stats = Format("{0} and {1} total", Plural($statData["numThreads"], "thread"), Plural($statData["numPosts"], "post"));
$stats .= "<br>".format("{0} today, {1} last hour", Plural($statData["newToday"], "new post"), $statData["newLastHour"]);
...
if (numRows($lastUser)) {
	...
	$last = format("{0}, {1} active ({2}%)", Plural($statData["numUsers"], "registered user"), $statData["numActive"], $percent)."<br>".format("Newest: {0}", UserLink($lastUser));
}
else
	$last ="No registered users<br>&nbsp;";
