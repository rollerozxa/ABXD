...
$crumbs = new PipeMenu();
$crumbs->add(new PipeMenuLinkEntry("Mood avatars", "editavatars"));
makeBreadcrumbs($crumbs);

AssertForbidden("editMoods");

if (!$loguserid)
	Kill("You must be logged in to edit your avatars.");
...
if (isset($_POST['action'])) {
	...
	if ($_POST['action'] == "Rename") {
		...
		if ($fname = $_FILES['picture']['name']) {
			...
			if (!$validext)
				$error.="<li>Invalid file type, must be one of: ".$extlist."</li>";
