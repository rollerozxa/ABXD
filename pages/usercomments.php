...
$crumbs->add(new PipeMenuTextEntry("Comments"));
...
if ($loguserid && ($_GET['token'] == $loguser['token'] || $_POST['token'] == $loguser['token'])) {
	...
	if (isset($_POST['actionpost']) && IsReallyEmpty($_POST['text']) && $canComment) {
		...
		logAction('usercomment', ['user2' => $id]);
...
//		$commentField = "You already have the last word.";
	if (!IsAllowed("makeComments") || !$canComment)
		$commentField = "You are not allowed to post usercomments.";
}
...
print "
				<table class=\"outline margin\">
					<tr class=\"header1\">
						<th colspan=\"2\">
							".format("Comments about {0}", UserLink($user))."
						</th>
					</tr>
					$commentList
					<tr>
